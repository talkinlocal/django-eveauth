...
def update(model_instance, *args):
    ...
    db_values = [f.get_db_prep_save(f.pre_save(model_instance, False),connection) for f in fields]
    if db_values:
        connection.cursor().execute("UPDATE %s SET %s WHERE %s=%%s" % \
            (connection.ops.quote_name(opts.db_table),
             ...
             db_values + opts.pk.get_db_prep_lookup('exact', model_instance.pk, connection))

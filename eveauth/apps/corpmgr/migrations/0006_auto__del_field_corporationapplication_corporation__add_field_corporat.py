...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'CorporationApplication', fields ['corporation', 'character']
        db.delete_unique('corpmgr_corporationapplication', ['corporation_id', 'character_id'])

        # Deleting field 'CorporationApplication.corporation'
        db.delete_column('corpmgr_corporationapplication', 'corporation_id')

        # Adding field 'CorporationApplication.corporation_profile'
        db.add_column('corpmgr_corporationapplication', 'corporation_profile',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='member_applications', to=orm['corpmgr.CorporationProfile']),
                      keep_default=False)

        # Adding unique constraint on 'CorporationApplication', fields ['character', 'corporation_profile']
        db.create_unique('corpmgr_corporationapplication', ['character_id', 'corporation_profile_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'CorporationApplication', fields ['character', 'corporation_profile']
        db.delete_unique('corpmgr_corporationapplication', ['character_id', 'corporation_profile_id'])


        # User chose to not deal with backwards NULL issues for 'CorporationApplication.corporation'
        raise RuntimeError("Cannot reverse this migration. 'CorporationApplication.corporation' and its values cannot be restored.")
        # Deleting field 'CorporationApplication.corporation_profile'
        db.delete_column('corpmgr_corporationapplication', 'corporation_profile_id')

        # Adding unique constraint on 'CorporationApplication', fields ['corporation', 'character']
        db.create_unique('corpmgr_corporationapplication', ['corporation_id', 'character_id'])
    ...
    models = {
        ...
        'corpmgr.corporationprofile': {
            ...
            'alliance_profile': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'member_corp_profiles'", 'null': 'True', 'to': "orm['corpmgr.AllianceProfile']"}),
